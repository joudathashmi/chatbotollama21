...
_INTENT_NOTE_TEMPLATE = {
    "executive_lookup": (
        ...
        "\n"
        "  ## Background\n"
        "  About the PERSON only (career arc, education, prior roles).\n"
        "  4-7 stable public bullets. Cap ~180 words. Do not contradict Role.\n"
        "  NEVER put company founding dates, product lists, MENA headcount,\n"
        "  or corporate revenue here — that is company_profile content.\n"
        "\n"
        "  ## 🇸🇦 Strategic Read\n"
        "  2-4 concrete MISA engagement angles anchored to THIS PERSON.\n"
        ...
        "FORBIDDEN SECTIONS for this intent (never emit these headings):\n"
        "  ## Saudi / MENA Position, ## Economic Outlook,\n"
        "  ## Policy & Regulatory, ## Engagement History,\n"
        "  ## MISA Contacts, ## Open Action Items,\n"
        "  ## Companies & Investors, ## <Company> — Executive Briefing,\n"
        "  ### 📊 Corporate Profile & Regional Footprint.\n"
        "If company rows appear in the payload, use them ONLY to name the\n"
        "employer in Role / one Strategic Read bullet — do not dump them.\n"
    ),
    "company_profile": (
        ...
        "  - In Snapshot of Operations, NAME the CEO from\n"
        "    `company_executives` (don't say 'the CEO').\n"
        "  - In Snapshot of Operations, NAME distributors / hubs /\n"
        "    academies / geo mix from MENA notes + geographic revenues.\n"
        "  - In the engagement angle, REFERENCE the most recent\n"
        "    meeting from `meetings` by date and outcome when present.\n"
        "Bottom Line Up Front, table-driven, ops-rich, strategy-forward.\n"
        "\n"
        "STRICT structure (Markdown) — headers VERBATIM, in this order:\n"
        "\n"
        ...
        "<MENA/Saudi revenue ONLY if an explicit MENA/KSA revenue "
        "field exists in the payload with a non-zero, non-global-"
        "duplicate value; otherwise write exactly: "
        ...
        "\n"
        "  HARD RULE — never invent MENA/Saudi revenue by applying a\n"
        "  geographic % to global revenue (e.g. do NOT write $39.1B as\n"
        "  10% of $391B). That is fabrication.\n"
        "\n"
        "  ---\n"
        "\n"
        "  ### Snapshot of Operations and Market Position\n"
        "\n"
        "  REQUIRED. 7–12 dense bullets woven from the payload\n"
        "  (company_profile + `_related` / correlator slices). Cover\n"
        "  what is present — skip what is not:\n"
        "  - Products / business units (devices, services, cloud, etc.)\n"
        "  - Geographic revenue mix when `company_geographic_revenues`\n"
        "    has real region shares (state shares; do NOT invent dollars)\n"
        "  - MENA entry / retail / hub / academy / distributor facts\n"
        "    from mena_notes / history_in_mena / entities\n"
        "  - Named leadership (CEO + key execs) from company_executives\n"
        "  - Competitive / financial KPIs when in payload (market cap,\n"
        "    margin, ESG) — omit if absent; do not invent\n"
        "  - Open opportunities / MISA contacts when present — NAME the\n"
        "    MISA contact from `misa_contact_details` when the payload\n"
        "    has one (do not invent a contact)\n"
        "\n"
        "  This section is the BODY of the briefing. Do NOT skip it.\n"
        "  Do NOT replace it with a thin '## Saudi / MENA Position' only.\n"
        "\n"
        "  ---\n"
        "\n"
        "  ### 🇸🇦 Strategic Read\n"
        ...
        "\n"
        "The company IS the headline. The table + Snapshot ARE the body.\n"
        "The Strategic Read IS what makes the briefing valuable.\n"
    ),
    "saudi_presence": (
    ...
    "engagement_strategy": (
        "INTENT: engagement_strategy — outreach guidance (ai_response_6 shape).\n"
        "\n"
        "STRUCTURE — headers VERBATIM, in this order:\n"
        "  ## Engagement Recommendation\n"
        "  - **Recommended approach:** <one crisp sentence>\n"
        "  - **Priority stakeholders:** <named roles / entities from record>\n"
        "  - **Why this matters to MISA:** <2-3 short bullets from record>\n"
        "  - **Talking points:** <3-5 concrete bullets from record>\n"
        "  - **Risks / unknowns:** <what to validate>\n"
        "\n"
        "  ## Snapshot\n"
        "  One short paragraph: sector, HQ, scale (revenue / employees)\n"
        "  from the record only.\n"
        "\n"
        "  ## Saudi / MENA Position\n"
        "  Bullets on KSA/MENA footprint, entities, RHQ, hubs, academies,\n"
        "  headcount — record only. No invented presence.\n"
        "\n"
        "  ## Strategic Read\n"
        "  2–4 concrete MISA engagement angles anchored to facts above.\n"
        "\n"
        "FORBIDDEN: '## … — Executive Briefing', Operational Detail tables,\n"
        "Corporate Profile & Regional Footprint tables, 'leverage' / 'explore'\n"
        "filler, invented competitor lists.\n"
    ),
    "financial_lookup": (
    ...
    "country_profile": (
        ...
        "  - company_profiles licensed/RHQ slices (not rhq_licenses):\n"
        "    REAL companies from this country with Saudi presence,\n"
        "    keyed by company_profiles.licensed / is_rhq (canonical).\n"
        "    Do NOT treat rhq_licenses (~661 rows) as the country total.\n"
        "\n"
        ...
        "  Sourced from `company_profiles.licensed` / `is_rhq`\n"
        "  (MISA's CANONICAL licensing markers — "
        "COUNT WHERE licensed=true / is_rhq=true). You may receive:\n"
        "    `company_profiles_rhq`       → is_rhq = true "
        "(Regional HQ holders — top tier)\n"
        "    `company_profiles_licensed`  → licensed = true, not RHQ\n"
        "         (Saudi licence but no full RHQ)\n"
        "    `company_profiles_unlicensed`→ licensed is not true\n"
        "         (present but not licensed; country from nationality)\n"
        "\n"
