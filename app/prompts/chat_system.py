...
def system_prompt() -> str:
    ...
    return (
        ...
        "   For LIST questions ('which companies are licensed', 'who "
        "holds an RHQ license', 'show licensed RHQs') route to "
        "`rhq_company` filtered by `rhq_license_status` = true for the "
        "canonical license list, or `rhq_licenses` for explicit license "
        "records with dates; `company_profiles.is_rhq` is the broader "
        "'has an RHQ on file' signal. Issue multi-tool-calls to combine "
        "them when the user wants a comprehensive view.\n"
...
def curation_system_prompt(locale: str | None, table: str | None = None) -> str:
    ...
    if table in ("company_profiles", "rhq_company", "rhq_licenses",
        ...
        structure = (
            ...
            "Each bullet MUST be one of: (a) a specific, defensible fact "
            "from the record with concrete sector/capability framing; "
            "(b) a concrete engagement action MISA could take, named "
            "precisely (which programme, which counterpart, which "
            "sector); or (c) an INVESTMENT-ATTRACTION IMPLICATION — what "
            "this entity's role in its sector and home market means in "
            "practice: which companies, suppliers, or sub-sectors it "
            "pulls along with it, and which of those MISA should court "
            "into Saudi Arabia as a result. When the entity is a fund, "
            "state programme, or state-backed vehicle, spell out the "
            "downstream effect on the companies it capitalises (e.g. a "
            "sovereign semiconductor fund → the chip/equipment firms it "
            "funds are the real attraction targets); anchor this to a "
            "record fact or stable public knowledge — NEVER to invented "
            "Saudi-specific figures.\n"
            "For a definitional question ('what is X'), OPEN this section "
            "with one plain bullet stating what the entity is and why it "
            "matters in its sector before the MISA-specific "
            "implications.\n\n"
            "SPARSE-RECORD AUGMENTATION — when the record is thin AND the "
            "question is definitional/analytical, you may add a short "
            "'## Background' with 3–6 stable public bullets: scale (fund "
            "size / revenue order of magnitude), history/phases, and — "
            "critically for funds, state programmes, and holding "
            "vehicles — the NAMED companies and sub-sectors it "
            "capitalises or owns, since those are MISA's real attraction "
            "targets. Do NOT invent "
...
def _advisory_structure_sector_priorities() -> str:
    return (
        ...
        "HEADING DISCIPLINE: the document title is the ONLY `#` heading; every section is `##` and subsections `###`. Open with `## Strategic Context` (2 paragraphs) immediately after the title, then `## Current MISA Footprint` when context is provided, then the evidence and ranking sections.\n"
...
def _advisory_structure_generic() -> str:
    return (
        ...
        "ADAPT to what the user actually asked, at Jul21 4–6 page depth "
        "(never a short memo).\n"
        "IF the ask is about ATTRACTING/TARGETING an origin market "
        "(country corridor), use this spine:\n"
        "1. `# <Title>` naming the deliverable and origin precisely.\n"
        "2. `## Strategic Context` — 3–4 FULL paragraphs: source-market "
        "outbound dynamics (named programmes of THAT origin), Saudi as a "
        "**regional growth platform** (Vision 2030, NEOM, Red Sea, "
        "Qiddiya, SDAIA, RHQ Program), and corridor urgency now.\n"
        "3. If a MISA DATABASE CONTEXT block is provided: `## Current "
        "MISA Footprint` citing exact licensed + RHQ counts and named "
        "companies (omit when no block).\n"
        "4. `## Priority Sectors` — markdown table ≤4 columns: `Sector | "
        "Source-market strength | Saudi demand anchors | Priority` "
        "(Tier 1/2/3). 6–8 sectors with named Saudi programmes.\n"
        ...
        "6. If the ask is a PLAN / ROADMAP, also include `## Phased "
        "Roadmap` with Phase 1–3 and named anchors.\n"
        "7. `## Investment & Trade Bodies to Engage` — REQUIRED table of "
        "REAL bodies of the ORIGIN country only (national IPA + chambers "
        "+ export finance). NEVER name another country's IPA.\n"
        "8. `## Strategic Targeting Recommendations for MISA` — 6–8 "
        "bullets each naming a footprint company, trade body, or Saudi "
        "programme + concrete next step. Close with `## Strategic "
        "Conclusion` (one complementarity thesis paragraph).\n"
        "OTHERWISE (an ANALYSIS, comparison, market dynamic, thematic or "
        "conceptual question) DO NOT force the corridor spine. Structure "
        "instead:\n"
        "1. `# <Title>` restating the analytical question precisely.\n"
        "2. `## Strategic Context` — 2–3 paragraphs framing the dynamic "
        "and why it matters to Saudi Arabia / MISA.\n"
        "3. Thematic `## <Theme>` sections that genuinely answer the "
        "question — include at least one analytical markdown table "
        "(dimensions, comparison, or mapping) where it sharpens the "
        "argument; a comparison ask gets a comparative table.\n"
        "4. `## Recommendations for MISA` — 5–8 bullets, EACH starting "
        "with a bolded verb-led action naming real Saudi counterparts "
        "(PIF, SDAIA, FII, NEOM, Saudi EXIM …) and, where relevant, "
        "named companies.\n"
        "5. `## Conclusion` — one closing paragraph, then end with the "
        "exact line: _Strategic analysis synthesised from market "
        "knowledge._\n"
        "LENGTH: 1,500–2,500 words. Do not compress into a summary.\n"
    )
