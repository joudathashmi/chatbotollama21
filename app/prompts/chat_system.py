...
def _advisory_structure_sector_priorities() -> str:
    return (
        ...
        "7. `## Investment & Trade Bodies to Engage` — REQUIRED. ONLY "
...
def _advisory_structure_company_targeting() -> str:
    return (
        ...
        "1. `# <Title>` — e.g. 'Targeting <Origin> Companies for "
        "Investment Attraction: Strategic List and Investment Thesis'.\n"
        "2. `## Executive Summary` — 4–6 bullets: licensed + RHQ counts "
        "from MISA DATABASE CONTEXT (exact figures), expansion vs "
        "new-entry split, and the top strategic recommendation.\n"
        "3. `## Strategic Context` — 2–3 paragraphs on THIS origin vs "
        "Vision 2030 (named source-market programmes + Saudi growth "
        "platform). Never borrow another country's policy labels.\n"
        "4. `## Current MISA Footprint` — REQUIRED when context is "
        ...
        "5. `## Priority Company Ranking` — ONE markdown table with "
        "EXACTLY these columns (keep theses SHORT in the table — one "
        "sentence — so the PDF stays readable):\n"
        "   Rank | Company | Sector | Saudi Presence | Target Type | "
        "Proposed Investment | Thesis (1 sentence) | MISA Action\n"
        "   Target Type = `expansion` (already licensed/RHQ) or "
        "`new_entry` (not yet in MISA footprint).\n"
        "   At least 6 of the top 8 rows MUST be taken from "
        "`expansion_targets` / top RHQ / top licensed lists in the "
        "MISA DATABASE CONTEXT. Do NOT replace them with generic famous "
        "brands absent from the footprint.\n"
        "6. `## Detailed Investment Theses` — for the top 6–8 companies, "
        "use `### <Company>` subsections with SHORT PARAGRAPHS "
        "(not one-line stubs) covering: Why this company; Why Saudi "
        "Arabia; Why now; Exact proposed investment "
        "(RHQ expansion, manufacturing, R&D centre, shared-services, "
        "logistics hub, JV, tech-transfer centre — not vague "
        "'partnership' or event attendance alone); Saudi demand/"
        "capability gap; Stakeholders/programmes; Next ask for MISA; "
        "Evidence (cite MISA footprint when expansion). Label "
        "'Requires validation' for unverified external claims.\n"
        "7. `## Investment & Trade Bodies to Engage` — markdown table: "
        "Organisation | Type | Role. REAL bodies of the ORIGIN country "
        "only (national IPA + chambers + export finance). Never include "
        "another country's IPA.\n"
        "8. `## Recommendations for MISA` — 6–10 company-specific, "
        "executable next actions (name the company + counterpart).\n"
        "9. `## Sources and Data Limitations` — distinguish MISA "
        "database figures, official/public facts, and analytical "
        "inference.\n"
        "HARD RULES:\n"
        "- Expansion vs new-entry MUST be labelled; do not mix silently.\n"
        "- Every thesis proposes a concrete investment, not only sales "
        "or event participation.\n"
        "- Do not invent supplier ecosystems, incentives, or project "
        "memberships without evidence; mark as Requires validation.\n"
        "- Prefer ≤5 columns in the ranking table for PDF. Put long "
        "prose in Detailed Investment Theses, not in table cells.\n"
        "LENGTH: Detailed theses alone should read like a Jul21 brief "
        "(≈1,500–2,200 words total document).\n"
    )
